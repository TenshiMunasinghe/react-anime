...
const AnimeItem: React.FC<ItemProps> = React.memo(
	({ anime }) => {
		...
		return (
			<li>
				...
				<div className='grid__links'>
					{haveLinks && (
						<span className='grid__link-text--none'>リンクなし</span>
					)}
					{public_url && (
	...
	(prev, next) =>
		...
		prev.anime.twitter_account === next.anime.twitter_account ||
		prev.anime.public_url === next.anime.public_url
)
